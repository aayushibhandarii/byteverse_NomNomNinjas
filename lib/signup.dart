...
class _signupPageState extends State<signupPage> {
  ...
  signUp(String email, String password)async{     //in this through email and password we will use email authentication services
      if (email=="" && password==""){
        ...
        try{
          usercredential = await FirebaseAuth.instance.signInWithEmailAndPassword(email: email, password: password).then((value){       // .then is used to tell what to do user created the account
            Navigator.push(context, MaterialPageRoute(builder: (context)=>MyHomePage(title: "You signup succesfully")));
